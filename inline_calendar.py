...
def init(chat_id, base_date, min_date, max_date,
    ...
    m_names = ['-']
    if not month_names:
        m_names.extend(['Січ', 'Лют', 'Бер', 'Кв', 'Тр', 'Чер', 'Лип', 'Сер', 'Вер', 'Жов', 'Лис', 'Груд'])
        #  m_names.extend(['Jan', 'Feb', 'Mar', 'Ap', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
    else:
    ...
    if not days_names:
        days_names = ['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Суб', 'Вс']
        #  days_names = ['Mon', 'Tue', 'Wed', 'Th', 'Fr', 'Sat', 'Sun']
